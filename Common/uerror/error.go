...
func (r LzError) Error() string {
	return fmt.Sprintf("ErrorCode: %d. Message: %s. StackTrace: %s", r.ErrorCode, r.Message, r.StackTrace)
}

func EncodeInternalServerLzError(err error) error {
	return EncodeLzError(HttpCodeInternalServerError, ErrorCodeInternalServerError, "", err)
}

func DecodeLzError(err error) *LzError {
	if lzError, ok := err.(LzError); ok {
		return &lzError
	}
	return nil
}

func EncodeLzError(httpCode, errorCode int, message string, err error) error {
	stackTraceStr := ""
	if err != nil {
		stackTraceStr = fmt.Sprintf("%v \n %v", err, stackTrace(2))
	}
	lzError := LzError{
		HttpCode:   httpCode,
		ErrorCode:  errorCode,
...
func NewErrorFieldWithErrorCode(key, messesage string, errorCode int) error {
	json := fmt.Sprintf("[{\"key\":\"%s\",\"message\":\"%s\",\"errorCode\":\"%d\"}]", key, messesage, errorCode)
